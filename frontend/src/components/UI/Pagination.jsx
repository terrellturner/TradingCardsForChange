			...
			<ul className={`flex h-full flex-row space-x-2 ${className}`}>
				{page > 1 && (
					<Link to={getPageUrl(page - 1)}>
						<li className="rounded bg-creased-khaki p-2 px-4">Prev</li>
					</Link>
				)}
				...
				{getVisiblePages().map((pageNum) => (
					<Link key={pageNum} to={getPageUrl(pageNum)}>
						<li
							className={`rounded bg-creased-khaki p-2 px-4 ${
								pageNum === page
									? 'border border-creased-khaki bg-noir-de-vigne text-creased-khaki'
									: ''
							}`}
						>
							{pageNum}
				...
				{page < pages && (
					<Link to={getPageUrl(page + 1)}>
						<li className="rounded bg-creased-khaki p-2 px-4">Next</li>
					</Link>
				)}
			</ul>
		)
